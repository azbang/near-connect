...
class I {
  ...
  callParentFrame(e, t) {
    ...
    return window.parent.postMessage({ type: "near-wallet-injected-request", id: n, method: e, params: t }, "*"), new Promise((s, o) => {
      const a = (i) => {
        i.data.type === "near-wallet-injected-response" && i.data.id === n && (window.removeEventListener("message", a), i.data.success ? s(i.data.result) : o(i.data.error));
      };
  ...
  async signIn(e) {
    const t = await this.callParentFrame("near:signIn", {
      ...
      addFunctionCallKey: e?.addFunctionCallKey
    });
    return Array.isArray(t) ? t : [t];
  }
  async signInAndSignMessage(e) {
    const t = await this.callParentFrame("near:signInAndSignMessage", {
      network: e?.network ?? this.connector.network,
      addFunctionCallKey: e?.addFunctionCallKey,
  ...
  async signAndSendTransaction(e) {
    const t = h(e.actions), n = { ...e, actions: t, network: e.network ?? this.connector.network };
...
class $ extends b {
  ...
  get footer() {
    ...
    const { icon: e, heading: t } = this.delegate.footer;
    return d`
      <div class="footer">
        ${e ? d`<img src="${e}" alt="${t}" />` : ""}
        <p>${t}</p>
  ...
  get dom() {
    return d`<div class="modal-container">
...
class W {
  ...
  checkPermissions(e, t) {
    ...
    if (e === "allowsOpen") {
      const n = u(t?.url || ""), s = this.manifest.permissions.allowsOpen;
      return !n || !s || !Array.isArray(s) || s.length === 0 ? !1 : s.some((a) => {
        const i = u(a);
  ...
  _onMessage = async (e, t) => {
    ...
    if (t.data.method === "storage.keys") {
      this.assertPermissions(e, "storage", t);
      const o = Object.keys(localStorage).filter((a) => a.startsWith(`${this.storageSpace}:`));
      n(o);
      return;
    }
    ...
    if (t.data.method === "walletConnect.getProjectId") {
      if (!this.connector.walletConnect) throw new Error("WalletConnect is not configured");
      this.assertPermissions(e, "walletConnect", t);
      const o = await this.connector.walletConnect;
      n(o.core.projectId);
      return;
    }
    ...
    if (t.data.method === "walletConnect.getSession") {
      ...
      try {
        if (!this.connector.walletConnect) throw new Error("WalletConnect is not configured");
        const o = await this.connector.walletConnect, a = o.session.keys[o.session.keys.length - 1], i = a ? o.session.get(a) : null;
    ...
    if (t.data.method === "open.nativeApp") {
      ...
      i.src = t.data.params.url, i.style.display = "none", document.body.appendChild(i), e.postMessage({ ...t.data, status: "success", result: null });
      return;
    }
  };
  actualCode = null;
  ...
  async call(e, t) {
    ...
    return new Promise((a, i) => {
      try {
        const l = (c) => {
          c.data.id !== o || c.data.origin !== s.origin || (s.dispose(), window.removeEventListener("message", l), this.connector.logger?.log("postMessage", { result: c.data, request: { method: e, params: t } }), c.data.status === "failed" ? i(c.data.result) : a(c.data.result));
        };
        window.addEventListener("message", l), s.postMessage({ method: e, params: t, id: o }), s.on("close", () => i(new Error("Wallet closed")));
      } catch (l) {
        this.connector.logger?.log("Iframe error", l), i(l);
      }
    });
  }
  async getAllStorage() {
...
class p {
  constructor(e, t) {
    this.connector = e, this.manifest = t, this.executor = new W(e, t);
  }
  executor;
  async signIn(e) {
...
class D {
  ...
  async signIn({ addFunctionCallKey: e, network: t }) {
    return this.wallet.signIn({
      network: t ?? this.connector.network,
      addFunctionCallKey: e
    });
  }
  async signInAndSignMessage(e) {
  ...
  async signAndSendTransaction(e) {
    const t = h(e.actions), n = e.network ?? this.connector.network, s = await this.wallet.signAndSendTransaction({ ...e, actions: t, network: n });
...
class F extends b {
  ...
  get footer() {
    if (!this.delegate.footer) return "";
    const { icon: e, heading: t, link: n, linkText: s } = this.delegate.footer;
    return d`
      <div class="footer">
        ${e ? d`<img src="${e}" alt="${t}" />` : ""}
        <p>${t}</p>
  ...
  get dom() {
    return this.state.showSettings ? d`
...
class j {
  ...
  async getItem(e) {
    ...
    return new Promise((n, s) => {
      ...
      const i = o.objectStore(this.storeName).get(e);
      i.onerror = (l) => s(i.error), i.onsuccess = () => {
        n(i.result), t.close();
      };
    });
  }
  async setItem(e, t) {
    ...
    return new Promise((s, o) => {
      ...
      a.onerror = (c) => o(a.error);
      const l = a.objectStore(this.storeName).put(t, e);
  ...
  async removeItem(e) {
    ...
    return new Promise((n, s) => {
      ...
      o.onerror = (l) => s(o.error);
      const i = o.objectStore(this.storeName).delete(e);
      i.onerror = (l) => s(i.error), i.onsuccess = () => {
        t.close(), n();
      };
    });
...
class q {
  ...
  constructor(e) {
    this.db = new j("hot-connector", "wallets"), this.storage = e?.storage ?? new k(), this.events = e?.events ?? new y(), this.logger = e?.logger, this.network = e?.network ?? "mainnet", this.walletConnect = e?.walletConnect, this.autoConnect = e?.autoConnect ?? !0, this.providers = e?.providers ?? { mainnet: [], testnet: [] }, this.excludedWallets = e?.excludedWallets ?? [], this.features = e?.features ?? {}, e?.footerBranding !== void 0 ? this.footerBranding = e?.footerBranding : this.footerBranding = {
      ...
      }), this.wallets = this.wallets.filter((n) => n.manifest.id !== t.data.manifest.id), this.wallets.unshift(new I(this, t.data.manifest)), this.events.emit("selector:walletsChanged", {}), this.autoConnect && this.connect({ walletId: t.data.manifest.id }));
  ...
  async switchNetwork(e, t) {
    ...
    }), this.network = e, await this.connect(t));
  }
  ...
  async connect(e = {}) {
    let t = e.walletId;
    const n = e.signMessageParams;
    await this.whenManifestLoaded.catch(() => {
    }), t || (t = await this.selectWallet({
      features: {
        signInAndSignMessage: e.signMessageParams != null ? !0 : void 0,
        signInWithFunctionCallKey: e.addFunctionCallKey != null ? !0 : void 0
      }
    }));
    try {
      const s = await this.wallet(t);
      this.logger?.log("Wallet available to connect", s), await this.storage.set("selected-wallet", t), this.logger?.log(`Set preferred wallet, try to signIn${n != null ? " (with signed message)" : ""}`, t);
      let o;
      if (e.addFunctionCallKey != null && (this.logger?.log("Adding function call access key during sign in with params", e.addFunctionCallKey), o = {
        ...e.addFunctionCallKey,
        gasAllowance: e.addFunctionCallKey.gasAllowance ?? {
          amount: "250000000000000000000000",
          // 0.25 NEAR in yoctoNEAR
          kind: "limited"
        }
      }), n != null) {
        const a = await s.signInAndSignMessage({
          addFunctionCallKey: o,
          messageParams: n,
          network: this.network
        });
        if (!a?.length) throw new Error("Failed to sign in");
        this.logger?.log("Signed in to wallet (with signed message)", t, a), this.events.emit("wallet:signInAndSignMessage", { wallet: s, accounts: a, success: !0 }), this.events.emit("wallet:signIn", {
          wallet: s,
          accounts: a.map((i) => ({
        ...
        const a = await s.signIn({
          addFunctionCallKey: o,
          network: this.network
        });
        if (!a?.length) throw new Error("Failed to sign in");
        this.logger?.log("Signed in to wallet", t, a), this.events.emit("wallet:signIn", { wallet: s, accounts: a, success: !0, source: "signIn" });
      }
      return s;
    } catch (s) {
      throw this.logger?.log("Failed to connect to wallet", s), s;
    }
  }
  async disconnect(e) {
